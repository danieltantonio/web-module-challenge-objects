...
console.log(createMenuItem('Fish with Rice', 18, "Lunch"));

/* Task 1b: use your function to create 3 more menu items. You may add any items to the menu that you'd like */
console.log(createMenuItem('Pizza', 5, "Lunch"));
console.log(createMenuItem('Maccoroni and Cheese', 7, "Lunch"));
console.log(createMenuItem('Chicken Nuggets', 5, "Lunch"));
...
burger.discount = function(status) {
  if(status.toLowerCase() === "teacher" || status.toLowerCase() === "student") {
    let remove = this.price * 0.25;
    this.price -= remove;
  } else if (status.toLowerCase() === "public") {
    let remove = this.price * 0.25;
    this.price -= remove;
  }
}
...
reviews.push(
  {
    name: "Daniel",
    rating: 1.5,
    feedback: "The only thing I liked was that fish and rice tho tbh."
  }
)
...
function getReviewByIndex(reviews, index) {
  ...
  return `${reviewer.name} gave the restaurant a ${reviewer.rating} and their feedback was :\n ${reviewer.feedback}`;
}
  
console.log(getReviewByIndex(reviews, reviews.length - 1));
...
function getLastReview(arr) {
    ...
    return `${review.name} gave the restaurant a ${review.rating} `;
} 
